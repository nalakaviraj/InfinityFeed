...
public function sendMessage($user_to,$body,$date){
	 ...
	 if($body !=""){
		...
		echo 	"<script>
				...
				</script>"; // this script avoids the form submission when refreshing the page.
...
public function getMessages($otherUser){
	...
	while($row=mysqli_fetch_array($get_messages_query)){
		...
		$div_top=($user_to==$userLoggedIn) ? "<div class='message' id='green'>" : "<div class='message' id='blue'>"; //short version of a if statement
...
public function getConvos(){
 	...
 	while ($row=mysqli_fetch_array($query)) {
 		$user_to_push=($row['user_to']!=$userLoggedIn) ? $row['user_to'] : $row['user_from']; //if user_to is not equal to userLoggedin then user_to_push= user_to else user_to_push=userfrom. Any way we have to get the user_to_push to be equal to the other user//
...
public function getConvosDropdown($data,$limit){
	...
	$userLoggedIn=$this->user_obj->getUsername();
 	...
 	foreach ($convos as $username) {
 		...
 		//Here in $is_unread_query the username is checked against userlogged in whether it is opened or not.if it is not opened a style part is added to the $style variable.
 		$row=mysqli_fetch_array($is_unread_query);
 		$style=($row['opened']=='no') ? "background-color:red;" : "";
 		$user_found_obj=new User($this->con,$username);
 		$latest_message_details=$this->getLatestMessage($userLoggedIn,$username);
